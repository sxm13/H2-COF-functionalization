...
def gbr(data_csv,unit, n_job, call, save = True):
    warnings.filterwarnings("ignore", category=FutureWarning, module='xgboost')
    Xtrain, Xtest, Ytrain, Ytest = normal(data_csv)
    reg = XGBRegressor()
...
def lasso(data_csv,unit, n_job, call, save = True):
    warnings.filterwarnings('ignore', category=ConvergenceWarning)
    Xtrain, Xtest, Ytrain, Ytest = normal(data_csv)
    reg = Lasso()
    ...
    print("Test: r2,mae,rmse: ", r2_test,mae_test,rmse_test)
    if save:
        joblib.dump(reg_opt,"lasso_" + unit + ".pkl")
        result = pd.ExcelWriter("lasso_"+unit+".xlsx")
        df_result_train = pd.DataFrame({"T": Xtrain["T"],
                                        "Ytrain": Ytrain.values.reshape(-1),
                                        'Ytrain_pre': reg_opt.predict(Xtrain).ravel()})
        df_result_train.to_excel(result, index=False, sheet_name = "data_train")
...
def kr(data_csv,unit, n_job, call, save = True):
    Xtrain, Xtest, Ytrain, Ytest = normal(data_csv)
    reg = KernelRidge()
    space = [
    ...
    print("Test: r2,mae,rmse: ", r2_test,mae_test,rmse_test)
    if save:
        joblib.dump(reg_opt,"kr_" + unit + ".pkl")
        result = pd.ExcelWriter("kr_"+unit+".xlsx")
        df_result_train = pd.DataFrame({"T": Xtrain["T"],
                                        "Ytrain": Ytrain.values.reshape(-1),
                                        'Ytrain_pre': reg_opt.predict(Xtrain).ravel()})
        df_result_train.to_excel(result, index=False, sheet_name = "data_train")
...
def ada(data_csv,unit, n_job, call, save = True):
    Xtrain, Xtest, Ytrain, Ytest = normal(data_csv)
    ...
    print("Test: r2,mae,rmse: ", r2_test,mae_test,rmse_test)
    if save:
        joblib.dump(reg_opt,"ada_" + unit + ".pkl")
        result = pd.ExcelWriter("ada_"+unit+".xlsx")
        df_result_train = pd.DataFrame({"T": Xtrain["T"],
...
def svr(data_csv,unit, n_job, call, save = True):
    Xtrain, Xtest, Ytrain, Ytest = normal(data_csv)
    Ytrain = Ytrain.values.ravel()
    Ytest = Ytest.values.ravel()
    reg = SVR()
    ...
    print("Test: r2,mae,rmse: ", r2_test,mae_test,rmse_test)
    if save:
        joblib.dump(reg_opt,"svr_" + unit + ".pkl")
        result = pd.ExcelWriter("svr_"+unit+".xlsx")
        df_result_train = pd.DataFrame({"T": Xtrain["T"],
                                        "Ytrain": Ytrain.reshape(-1),
                                        'Ytrain_pre': reg_opt.predict(Xtrain).ravel()})
        df_result_train.to_excel(result, index=False, sheet_name = "data_train")
...
def knn(data_csv,unit, n_job, call, save = True):
    Xtrain, Xtest, Ytrain, Ytest = normal(data_csv)
    ...
    print("Test: r2,mae,rmse: ", r2_test,mae_test,rmse_test)
    if save:
        joblib.dump(reg_opt,"knn_" + unit + ".pkl")
        result = pd.ExcelWriter("knn_"+unit+".xlsx")
        df_result_train = pd.DataFrame({"T": Xtrain["T"],
                                        "Ytrain": Ytrain.values.reshape(-1),
                                        'Ytrain_pre': reg_opt.predict(Xtrain).ravel()})
        df_result_train.to_excel(result, index=False, sheet_name = "data_train")
...
def mlp(data_csv, unit, n_job, call, save = True):
    warnings.filterwarnings('ignore', category=ConvergenceWarning)
    Xtrain, Xtest, Ytrain, Ytest = normal(data_csv)
    Ytrain = Ytrain.values.ravel()
    Ytest = Ytest.values.ravel()
    ...
    if res_gp.x[0]== 'sgd':
        ...
        print("Test: r2,mae,rmse: ", r2_test,mae_test,rmse_test)
    if save:
        joblib.dump(reg_opt,"mlp_" + unit + ".pkl")
        result = pd.ExcelWriter("mlp_"+unit+".xlsx")
        df_result_train = pd.DataFrame({"T": Xtrain["T"],
                                        "Ytrain": Ytrain.reshape(-1),
                                        'Ytrain_pre': reg_opt.predict(Xtrain).ravel()})
        df_result_train.to_excel(result, index=False, sheet_name = "data_train")
