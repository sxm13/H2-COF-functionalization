...
def predict(train_csv, test_csv, root_dir, model_path, all_csv):
    ...
    for _, (input, target, name) in enumerate(data_loader_train):
        input_var = (
                Variable(input[0]),
                Variable(input[1]),
                input[2],
    ...
    test_pre = []
    for _, (input, target, name) in enumerate(data_loader_test):
        input_var = (
                Variable(input[0]),
                Variable(input[1]),
                input[2],
...
def predict_all(train_csv, test_csv, root_dir, model_path, all_csv):
    from model.CGCNN_data_pre import CIFData_all,get_train_val_test_loader,collate_pool
    dataset_all  = CIFData_all(root_dir=root_dir, data_file=all_csv)
    sample_data_list = [dataset_all[i] for i in range(len(dataset_all))]
    _, sample_target, _ = collate_pool(sample_data_list)
    normalizer = Normalizer(sample_target)

    # dataset_train = CIFData_all(root_dir=root_dir, data_file=train_csv)
    # structures, _, _ = dataset_train[0]
    # orig_atom_fea_len = structures[0].shape[-1]
    # nbr_fea_len = structures[1].shape[-1]
    ...
    dataset_test = CIFData_all(root_dir=root_dir, data_file=test_csv)
    structures, _, _ = dataset_test[0]
    orig_atom_fea_len = structures[0].shape[-1]
    nbr_fea_len = structures[1].shape[-1]
    model = CrystalGraphConvNet(orig_atom_fea_len, nbr_fea_len,n_conv = 3,n_out=3)
